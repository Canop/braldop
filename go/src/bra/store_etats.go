...
func (con ConnexionMysql) EtatBraldun(idBraldun uint) (*EtatBraldun, error) {
	row := con.QueryRow("select pv, pvmax, pa, tour, tour, dla, faim from compte where id=?", idBraldun)
...
func (con ConnexionMysql) StockeEtatBraldun(e *EtatBraldun) error {
	sql := "update compte set pv=?, pvmax=?, pa=?, tour=?, dla=?, faim=? where id=?"
	res, err := con.Exec(sql, e.PV, e.PVMax, e.PA, e.DuréeTour, e.DLA, e.Faim, e.IdBraldun)
	if res != nil {
