...
nohup go/bin/braldopserver ${MAPSERVER_ARGS} -mysqluser ${USER_MYSQL} -mysqlmdp ${MDP_MYSQL} -cartes $CHEMIN_REPERTOIRE_DONNEES/cartes >> log/chrallserver.log 2>&1 < /dev/null &
